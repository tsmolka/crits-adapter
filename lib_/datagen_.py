...
def get_random_spam_msg(config):
    header_map = {'Subject': 'subject', 'To': 'to',
                  'Cc': 'cc', 'Bcc': 'bcc', 'From': 'from_',
                  'Sender': 'sender', 'Date': 'date',
                  'Message-ID': 'message_id', 'Reply-To': 'reply_to',
                  'In-Reply-To': 'in_reply_to', 'Content-Type': 'content_type',
                  'Errors-To': 'errors_to', 'Precedence': 'precedence',
                  'Boundary': 'boundary', 'MIME-Version': 'mime_version',
                  'X-Mailer': 'x_mailer', 'User-Agent': 'user_agent',
...
def get_email_payload(msg):
    val = None
    # msg has no body
    if not hasattr(msg, 'get_content_maintype'):
        return(val)
    ...
    if type_ == 'multipart':
        ...
        for part in msg.get_payload():
            ...
            if not hasattr(part, 'get_content_type_'):
                continue
...
def generate_random_domain(config):
    '''generate a random domain name by pairing a random uuid with a
    random tld'''
    domain = str(uuid.uuid4())
    tld_index = random.randint(0, len(config['datagen']['tlds']) - 1)
    tld = config['datagen']['tlds'][tld_index]
    domain += '.%s' % tld
    return(domain)
